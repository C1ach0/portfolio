<template>
    <NuxtLayout>
                    ...
                    <div ref="projectContent" class="space-y-6">
                        ...
                        <div class="space-y-3">
                            <p class="text-lg text-gray-800 dark:text-gray-300 leading-relaxed transform translate-y-8 opacity-0"
                                ...
                                illustrant ma capacité à répondre à des problématiques variées et à relever des défis
                                techniques concrets.
                            </p>
                            <p class="text-lg text-gray-800 dark:text-gray-300 leading-relaxed transform translate-y-8 opacity-0"
                                ref="aboutText2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <CardAbout ref="skill1" class="skill-card">
                                <h4
                                    ...
                                    Projets</h4>
                                <p class="text-gray-800 dark:text-gray-300 text-sm">{{ projects.length }} réalisations
                                </p>
                            </CardAbout>
        ...
        <section class="py-20 px-6 max-w-screen-2xl mx-auto">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6" v-nppt="{
                step: 1,
                title: 'Projets réalisé (entreprise + perso)',
                keywords: [
                    { label: 'Projets réalisés', tone: 'info', size: 'lg' },
                    ...sortedProjects.map(pro => ({ label: pro.name, tone: 'success', size: 'sm' }))
                ],
                next: '/projets/ensmbpn-gipe'
            }">
                <div v-for="(project, index) in sortedProjects" :key="index" :id="`p_${project.slug}`"
                    class="rounded-2xl overflow-hidden bg-gradient-to-br from-gray-100/70 to-gray-200/60 dark:from-gray-800/70 dark:to-gray-900/60 border border-gray-100 dark:border-gray-700 hover:border-rose-500/50 shadow-md hover:shadow-rose-500/10 transition-all duration-300 group relative flex flex-col justify-between">
